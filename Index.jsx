...
export default function Home({ data }) {
    ...
    // Mobile: follow scroll — whichever card is nearest viewport center gets focus sizing
    useEffect(() => {
        if (isDesktop) return;
        const strip = bannerStripRef.current;
        ...
        strip.addEventListener('scrollend', updateBannerFocusFromScroll);
        window.addEventListener('resize', schedule);
        schedule();
        return () => {
            strip.removeEventListener('scroll', schedule);
            strip.removeEventListener('scrollend', updateBannerFocusFromScroll);
            window.removeEventListener('resize', schedule);
            if (raf) cancelAnimationFrame(raf);
        };
    }, [isDesktop, bannerCards.length, updateBannerFocusFromScroll]);

    // Mobile: default scroll so the middle banner card is centered in the strip
    useEffect(() => {
        if (isDesktop) return;
        const strip = bannerStripRef.current;
        ...
        const run = () => {
            try {
                ...
                const target = cardLeft - (strip.clientWidth - cardW) / 2;
    ...
    }, [isDesktop, bannerCards, updateBannerFocusFromScroll]);
    ...
    const handleSpokenStoryPublishMessage = useCallback(async () => {
        ...
        try {
            ...
            spokenStoryResetTimerRef.current = setTimeout(() => {
                ...
                // Reset for next Spoken Story recording (after user sees the message)
                setSpokenStoryBlob(null);
                setSpokenStoryDuration(0);
                setSpokenStoryShowActions(false);
                setSpokenStoryShowPublishButton(false);
                                        ...
                                        <div className="os-hero-beyond__cta-wrap mt-2 w-full px-0 flex flex-wrap items-center justify-center gap-3">
                                            {!auth?.user?.id && (
                                                <Link href={route('register')}>
                                                    <a
                                                        className="os-hero-beyond__cta-btn os-hero-beyond__cta-btn--primary signup-btn-larger"
                                                    >
                                                        Sign Up
                                                    </a>
                                                </Link>
