...
function IDinfo(id) {
  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      const avatarPath = data.user_avatar.replace('/storage/storage/', '/storage/');

        $(".messenger-infoView")
          .find(".avatar")
          .css("background-image", 'url("' + avatarPath + '")');
        $(".header-avatar").css(
          "background-image",
          'url("' + avatarPath + '")'
        );
    ...
    $.ajax({
      ...
      success: (data) => {
        if (data.error > 0) {
          ...
          // make seen saved message
          if (auth_id === getMessengerId()) {
              makeSeen(true);
          }
        }
      },
...
function openModal() {
    app_modal({
        show: true,
        name: "users",
        body: '<ul id="userList"><p>Loading...</p></ul>'
    });
    loadUsers();
}
        ...
        success: function(response) {
            ...
            if (response.users && response.users.length > 0) {
                response.users.forEach(user => {
                    const userAvatar = user.avatar ? user.avatar : '/img/avatar.png';

                    const userItem = `
                        <table class="messenger-list-item" data-contact="${user.id}" onclick="closeModal()">
                            <tr data-action="0">
                                <td style="position: relative">
                                    <div class="avatar av-m" style="background-image: url('${userAvatar}');"></div>
                                </td>
                                <td>
                                    <p data-id="${user.id}" data-type="user">
                                        ${(user?.name && user?.name != 'null') ? user?.name : user?.username}
                                    </p>
                                    <span class="lastMessageIndicator"></span>
                                </td>
                            </tr>
                        </table>
                    `;
                ...
                userList.innerHTML = '<p>There are no users</p>';
            }
        },
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const toggleBodyScroll = () => {
        const isDropdownOpen = document.querySelector('.dropdown__content--is-visible');
        document.body.classList.toggle('os-body--dropdown-open', !!isDropdownOpen);
    };

    dropdowns.forEach((dropdown) => {
        const trigger = dropdown.querySelector('.dropdown__trigger');
        const dropdownMenu = dropdown.querySelector('.dropdown__content');
        const dropdownClose = dropdown.querySelector('.os-dropdown__close');
        ...
        trigger.addEventListener('click', function (event) {
            ...
            document.querySelectorAll('.dropdown__content--is-visible').forEach((openMenu) => {
                if (openMenu !== dropdownMenu) {
                    openMenu.classList.remove('dropdown__content--is-visible');
                }
            });

            const isDropdownVisible = dropdownMenu.classList.toggle('dropdown__content--is-visible');

            if (dropdownClose) {
                dropdownClose.classList.toggle('os-dropdown__close--is-visible', isDropdownVisible);
            }
        ...
        document.addEventListener('click', function (event) {
            if (!dropdown.contains(event.target)) {
                dropdownMenu.classList.remove('dropdown__content--is-visible');
                if (dropdownClose) {
                    dropdownClose.classList.remove('os-dropdown__close--is-visible');
                }
