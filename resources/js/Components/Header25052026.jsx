...
export default function Header() {
    ...
    useEffect(() => {
        document.body.classList.toggle('os-body--dropdown-open', isUserMenuOpen);
        return () => {
    ...
    const handleToOpenVideoEditor = async (e) => {
        ...
        if (auth.user) {
            ...
            if (Array.isArray(media) && media.length > 0) {
                //localStorage.setItem("is_media", media);
                //window.location.href = 'http://127.0.0.1:8000/draft';
                window.location.href = 'https://onestoryplanet.com/draft';
            } else {
                //localStorage.setItem("not_media", media);
                try {
