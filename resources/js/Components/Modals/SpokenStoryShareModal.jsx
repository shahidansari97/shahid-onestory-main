...
export default function SpokenStoryShareModal({ spokenStoryRecording, onShareRecorded }) {
    const [copied, setCopied] = useState(false);
    const [nativeBusy, setNativeBusy] = useState(false);

    const spokenId = useMemo(() => {
        if (!spokenStoryRecording) return null;
        return typeof spokenStoryRecording === "object" ? spokenStoryRecording.id : spokenStoryRecording;
    }, [spokenStoryRecording]);

    const link = useMemo(() => {
        const id = spokenId;
        return id != null
            ? `${window.location.origin}/spoken-stories/${id}`
            : window.location.origin;
    }, [spokenId]);

    const recordShare = useCallback(async () => {
        if (spokenId == null) return;
        try {
            await axios.post(
                route("spoken-stories.share"),
                { spoken_story_recording_id: spokenId },
                { headers: { Accept: "application/json", "X-Requested-With": "XMLHttpRequest" } }
            );
            onShareRecorded?.(spokenId);
        } catch (e) {
            console.error("Spoken story share count failed:", e);
        }
    }, [spokenId, onShareRecorded]);
    ...
    const handleShare = (platform) => {
        ...
        switch (platform) {
            ...
            case "facebook":
                ...
                break;
            case "mail":
                shareUrl = `mailto:?subject=Check out this spoken story&body=${encodedLink}`;
                break;
            case "telegram":
    ...
    const handleNativeShare = async () => {
        ...
        try {
            await navigator.share({
                title: "Spoken story",
                text: "Check out this spoken story on OneStoryPlanet",
                url: link,
            });
            await recordShare();
        } catch (err) {
            ...
            {canNativeShare && (
                <div className="mb-4">
                    <Button
                        ...
                        disabled={nativeBusy || spokenId == null}
                    >
