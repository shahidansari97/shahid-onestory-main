...
export default function KeywordSelector({ selected = [], onChange, name, description, placeholder, onKeywordAdded }) {
    const [inputValue, setInputValue] = useState('');
    const [selectedKeywords, setSelectedKeywords] = useState([]);

    useEffect(() => {
        if (JSON.stringify(selectedKeywords) !== JSON.stringify(selected)) {
            setSelectedKeywords(Array.isArray(selected) ? selected : []);
        }
    }, [selected]);
    ...
    const addKeyword = (keyword) => {
        if (!selectedKeywords.includes(keyword)) {
            ...
            if (typeof onKeywordAdded === 'function') {
    ...
    return (
        <div className="os-keyword-selector">
            <TextInput
