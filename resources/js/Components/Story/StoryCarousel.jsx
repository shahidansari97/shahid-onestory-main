...
export default function Carousel({ items, children, isFocusedMode, disableAutoScroll = false }) {
    ...
    const staticItems = useMemo(() => {
        ...
        const itemsWithMedia = items.filter((item) => {
            const hasVideo = Boolean(item?.master_url || item?.src || item?.video_url);
            const hasThumb = Boolean(item?.thumbnail);
            return hasVideo && hasThumb;
        });
        return itemsWithMedia.slice(0, 4);
    }, [items]);

    // Mobile-only: Limit to 15 videos for mobile responsive version
    const mobileInfiniteItems = useMemo(() => {
        if (!items || items.length === 0 || isFocusedMode) return items;
        const limitedItems = items.slice(0, 15);
        return [...limitedItems, ...limitedItems, ...limitedItems];
    }, [items, isFocusedMode]);

    // Memoize settings to prevent recreation on every render
    const settings = useMemo(() => ({
        infinite: true,
        speed: 3000, // Faster: 3 seconds (was 5000)
        slidesToShow: 6,
        slidesToScroll: 1,
        autoplay: true,
        autoplaySpeed: 0,
        ...
        responsive: [
            {
                breakpoint: 1170,
                settings: { slidesToShow: 3, slidesToScroll: 1, speed: 3000 },
            },
            {
                breakpoint: 1000,
                settings: { slidesToShow: 2, slidesToScroll: 1, speed: 3000 },
            },
            {
                breakpoint: 650,
                settings: {
                    ...
                    slidesToScroll: 1,
                    centerPadding: "0px",
                    speed: 3000, // Same faster speed for mobile
                },
            },
        ],
    }), []);
    ...
    if (disableAutoScroll) {
        return !isFocusedMode ? (
            <div className="os-carousel os-carouselMain os-carouselMain--static">
                <div className="carousel-track carousel-track--static">
            ...
            !isFocusedMode ? (
                ...
                >
                    <Marquee speed={50} gradient={false} pauseOnHover={true}>
                        ...
                        {mobileInfiniteItems.map((item, index) => (
                            <div
                                key={`${item.id}-${index}`}
        ...
        !isFocusedMode ? (
            ...
            >
                <div className="carousel-track" ref={trackRef}>
                    {(isMobile ? mobileInfiniteItems : infiniteItems).map((item, index) => (
                        <MemoizedStoryItem 
                            key={`${item.id}-${index}`}
                            item={item} 
                            index={index} 
