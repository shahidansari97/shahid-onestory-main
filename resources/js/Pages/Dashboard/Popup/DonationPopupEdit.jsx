...
export default function DonationPopupEdit({ auth, data }) {
    ...
    const handleSubmit = (e) => {
        ...
        router.post(route('admin.donation-popup.update'), formData, {
            onError: (errors) => {
                if (errors) {
    ...
    return (
        <AuthenticatedLayout>
            <Wrapper title='Edit Donation Popup' message={message}>
                <Form className="flex flex-col p-5 gap-[24px] box box--stacked" onSubmit={handleSubmit}>
                    ...
                    <Button type="submit">Update Donation Popup</Button>
                </Form>
            </Wrapper>
        </AuthenticatedLayout>
    );
